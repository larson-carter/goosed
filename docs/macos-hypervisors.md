# PXE Booting macOS Hypervisor Guests

Use the optional `pxe-stack` helpers to bring DHCP, TFTP, and a PXE-friendly HTTP endpoint into your cluster so macOS hypervisors can boot guests directly from goose'd. The instructions below cover both VMware Fusion and Oracle VirtualBox.

## 1. Enable the PXE stack in Kubernetes
...
## 2. Configure your macOS hypervisor

### VMware Fusion

1. Create a new VM with no installation media.
2. Edit the network adapter and choose **Bridged (Autodetect)** so the guest shares the same layer-2 segment as the `pxe-stack` service. When you have multiple physical interfaces, explicitly bridge to the interface that reaches the PXE VLAN.
3. Power on the VM and press **Esc** (UEFI) or **F12** (BIOS) to pick **EFI Network**/**Network Boot**. You should see the VM obtain an address from the DHCP handler, download `undionly.kpxe` from TFTP, then chainload into `bootd`.

### Oracle VirtualBox

1. Create a new VM without attaching an ISO so the firmware falls back to network boot.
2. In **Settings → Network**, set **Adapter 1** to **Bridged Adapter**. Select the physical interface that can reach your PXE VLAN and enable **Promiscuous Mode: Allow All** if your upstream switch requires it for DHCP broadcasts.
3. Ensure **Cable Connected** remains checked, then boot the VM. Press **F12** when prompted and choose **PXE boot** (or **EFI Network** on UEFI firmware). The guest should follow the same DHCP → TFTP → `bootd` chainload flow as Fusion.

## 3. Serve installation media from SeaweedFS

Once your hypervisor can boot from the network, provide installation media through SeaweedFS so the Kickstart workflow can complete.
